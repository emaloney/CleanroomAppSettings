...
Once a device-based run destination has been selected, select the “CleanroomAppSettings” build scheme.
...
If all went well, the Carthage build directory should contain the file `CleanroomAppSettings.framework`. If that file isn’t present, something went wrong with the build.
