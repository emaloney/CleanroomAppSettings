...
*Integration* is the act of embedding the `CleanroomAppSettings.framework` binary into your project, thereby exposing the API it provides to your code.

CleanroomAppSettings is built as a *Swift framework*, and as such, it has the following base platform requirements:
...
**This is the `swift3` branch of CleanroomAppSettings.** It uses **Swift 3.0** and **requires Xcode 8** to compile.
...
- **[Carthage integration](#carthage-integration)** — Explains how to use the [Carthage](https://github.com/Carthage/Carthage) dependency manager for adding CleanroomAppSettings to your project.

- **[Manual integration](#manual-integration)** — Demonstrates the steps for adding CleanroomAppSettings by embedding `CleanroomAppSettings.xcodeproj` within your own Xcode project. (**Note:** Manual integration is a bit more involved than using Carthage).
...
When building a framework for a platform with a simulator, Carthage creates a *universal binary*, allowing `CleanroomAppSettings.framework` to work in the simulator as well as on actual devices.
...
The command `carthage update` causes Carthage to download and build CleanroomAppSettings.

By default, `carthage update` builds all platform targets in a project. Normally, this is _not_ what you want; you’ll usually use CleanroomAppSettings on just one platform at a time.
...
Once Carthage is done building CleanroomAppSettings, you can use the `open` command to locate the framework binaries in Finder:
...
If all went well, the Carthage build directories should contain the file `CleanroomAppSettings.framework`. If that file isn’t present, something went wrong with the build.
...
Go back to Finder, select the file `CleanroomAppSettings.framework`, and then drag it into the list area directly below *Embedded Binaries*.
...
But before you start coding, skip to the [Adding the Swift import](#adding-the-swift-import) section to see how you can import CleanroomAppSettings for use in your Swift code.
...
Next, you’re ready to [embed the `CleanroomAppSettings.xcodeproj` project file in your Xcode project](#2-embed-cleanroomappsettings-in-your-project).
...
Once a device-based run destination has been selected, select the appropriate build scheme for the target platform: “CleanroomAppSettings-iOS”, “CleanroomAppSettings-macOS”, “CleanroomAppSettings-tvOS” or “CleanroomAppSettings-watchOS”.
...
1. Locate `CleanroomAppSettings.framework` in the Xcode project browser
...
Once you’ve done this for each framework, **_you’re all done integrating CleanroomAppSettings!_**
...
Want to learn more about CleanroomAppSettings? Check out [the README](https://github.com/emaloney/CleanroomAppSettings/blob/swift3/README.md).
