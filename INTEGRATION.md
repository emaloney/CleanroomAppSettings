...
*Integration* is the act of embedding the `CleanroomAppSettings.framework` binary into your project and exposing the API it provides to your code.

Note that CleanroomAppSettings is built as a *Swift framework*. This has several implications, not the least of which is that it will only work on iOS 8 or above. It is also only supported for use by other Swift code. Some, all or none of it may work from Objective-C; we haven't tried it, we wouldn't recommend it, and we don't support it.
...
CleanroomAppSettings requires a **mimimum Xcode version of 6.3** to be built, and the resulting binary can be used on **iOS 8.1 and higher**.

We'll also be using the `git` command in the Terminal for installing the CleanroomAppSettings repo in your codebase. These steps have been tested with **git 2.3.2 (Apple Git-55)**, although they should be compatible with a wide range of git versions.
...
- **[Manual integration](#manual-integration)** — The `CleanroomAppSettings.xcodeproj` Xcode project file is embedded directly within your project. You then add `CleanroomAppSettings.framework` to the *Embedded Binaries* and *Linked Frameworks and Libraries* sections under the *General* tab for your application target.

- **[Carthage integration](#carthage-integration)** — [Carthage](https://github.com/Carthage/Carthage) is a dependency package manager designed to build frameworks. Once Carthage is installed, to add CleanroomAppSettings to your project using Carthage, you would put the line `github "emaloney/CleanroomAppSettings"` in your `Cartfile` and then issue the command `carthage update`.
...
Manual integration involves embedding `CleanroomAppSettings.xcodeproj` directly in your Xcode project.

This will ensure that CleanroomAppSettings is built with the exact same settings you’re using for your app. You won’t have to fiddle with different settings for different architectures — when you're running in the simulator, it will work; when you then switch to building for device, it'll work, too.

You’ll also be able to step into CleanroomAppSettings code directly in the debugger without worrying about `.dSYM` resolution, which is very helpful.

### An Overview of the Process

Manual integration is a bit involved; there are five high-level tasks that you'll need to perform:

1. Download the CleanroomAppSettings source into your project structure

2. Embed `CleanroomAppSettings.xcodeproj` in your Xcode project

3. Build `CleanroomAppSettings.framework`

4. Add `CleanroomAppSettings.framework` to your application target
...
For our integration examples, we're going to be showing the top-level `CleanroomAppSettings` directory inside a `Libraries` directory at the root level of of your application's source.
...
### 1. Download the CleanroomAppSettings source

If you're already using git for version control, we recommend adding CleanroomAppSettings to your project as a submodule. This will allow you to "lock" your codebase to specific versions of CleanroomAppSettings, making it easier to incorporate new versions on whatever schedule works best for you.

If you're using some other form of version control of if you're not using version control at all—*shame on you!*—then you'll want to *clone* the CleanroomAppSettings repository. We suggest putting the CleanroomAppSettings clone somewhere within your application's directory structure, so that it is included in whatever version control regimen you're using.

#### Downloading CleanroomAppSettings as a submodule

> **Important:** Skip this section if you plan to download CleanroomAppSettings using `git clone`.

From within the `Libraries` directory, issue the following command to download CleanroomAppSettings:

```bash
git submodule add https://github.com/emaloney/CleanroomAppSettings.git
```

Next, you're ready to [embed the `CleanroomAppSettings.xcodeproj` project file in your Xcode project](#2-embed-cleanroomasl-in-your-project).

#### Downloading CleanroomAppSettings as a cloned repo

> **Important:** Skip this section if you already performed the tasks outlined in "Downloading CleanroomAppSettings as a submodule" above.
...
In the Terminal, the command `open CleanroomAppSettings` to open the folder containing the CleanroomAppSettings source in the Finder. This will reveal the `CleanroomAppSettings.xcodeproj` Xcode project and all files needed to build `CleanroomAppSettings.framework`.

Then, open your application in Xcode, and drag `CleanroomAppSettings.xcodeproj` into the Xcode project browser. This will embed CleanroomAppSettings in your project and allow you to add the targets built by CleanroomAppSettings to your project.

### 3. Build CleanroomAppSettings

Before we can add `CleanroomAppSettings.framework` to your app, we have to build it, so Xcode has more information about the framework.
...
Once a device-based run destination has been selected, select the "CleanroomAppSettings" build scheme. Then, select *Build* (⌘B) from the *Product* menu.

Once the build is complete, open `CleanroomAppSettings.xcodeproj` in the project navigator and find the "Products" group. Open that, and right-click on `CleanroomAppSettings.framework`. Select *Show in Finder*. This will open the folder containing the framework binary you just built.

If all went well, you should see several files in this folder; the one we're concerned with is:

- `CleanroomAppSettings.framework`
...
Go back to Finder and drag `CleanroomAppSettings.framework` into the list area directly below *Embedded Binaries* in Xcode.

If successful, you should see `CleanroomAppSettings.framework` listed under both the *Embedded Binaries* and *Linked Frameworks and Libraries* sections.
...
So, to make things sane again, you'll need to make sure Xcode references `CleanroomAppSettings.framework` using a "Relative to Build Products" location:
...
Once you've done this, **_you're all done integrating CleanroomAppSettings!_**

Skip to the [Adding the Swift import](#adding-the-swift-import) section to see how you can import CleanroomAppSettings for use in your Swift code.
...
1. Update the `Cartfile` with an entry for CleanroomAppSettings
2. Download and build CleanroomAppSettings
3. Add `CleanroomAppSettings.framework` to your application target
...
```
github "emaloney/CleanroomAppSettings"
```
...
This will cause Carthage to download and build CleanroomAppSettings.
...
Once Carthage is done building CleanroomAppSettings and its dependencies, you can execute the following Terminal command to see the frameworks built by Carthage:

```bash
open Carthage/Build/iOS
```

This will cause the directory containing `CleanroomAppSettings.framework` to open in Finder.
...
Once you've done this, try building your application. If you don't see any errors, **_you're all done integrating CleanroomAppSettings!_**

## Adding the Swift import

Once CleanroomAppSettings has been successfully integrated, all you will need to do is add the following `import` statement to any Swift source file where you want to use CleanroomAppSettings:

```swift
import CleanroomAppSettings
```

Want to read more about CleanroomAppSettings? Check out [the README](https://github.com/emaloney/CleanroomAppSettings/blob/master/README.md).
