...
{
	case $1 in
	iOS) 		echo "platform=iOS Simulator,OS=10.2,name=iPhone 7";;
...
while [[ $THIS_TRY < $MAXIMUM_TRIES ]]; do
	...
	( set -o pipefail && xcodebuild -project CleanroomAppSettings.xcodeproj -configuration Debug -scheme "CleanroomAppSettings" -destination "$DESTINATION" -destination-timeout 300 $XCODE_ACTION 2>&1 | tee "CleanroomAppSettings-$PLATFORM-$OPERATION.log" | xcpretty )
	XCODE_RESULT="${PIPESTATUS[0]}"
	if [[ "$XCODE_RESULT" == "0" ]]; then
		rm "CleanroomAppSettings-$PLATFORM-$OPERATION.log"
		exit 0
